    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mr-auto">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-teal-700">Base de clientes</p>
          <h2 className="mt-1 text-lg font-semibold text-slate-950">Todos os contatos</h2>
        </div>
        ...
        <select
          ...
          className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700 outline-none ring-teal-600 focus:ring-2 sm:w-48"
        ...
        {importMessage && (
          <span className="basis-full text-sm text-slate-600">{importMessage}</span>
        )}
              ...
              {clients.map((client) => (
                ...
                >
                  <td className="px-3 py-2 font-medium text-zinc-900 dark:text-zinc-50">
                    <Link href={`/clients/${client.id}`} className="text-teal-800 hover:underline">
                      {client.name}
                    </Link>
                  </td>
                  ...
                  <td className="px-3 py-2 text-zinc-600 dark:text-zinc-300">
                    ...
                    {client.reviewsCount ? ` (${client.reviewsCount})` : ""}
                  </td>
                  <td className="px-3 py-2 text-zinc-600 dark:text-zinc-300">
                    {client.website ? (
                      <a
                  ...
                  <td className="px-3 py-2">
                    <select
                      ...
                      className="rounded-md border border-zinc-300 bg-white px-2 py-1 text-xs text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
                    >
                  ...
                  <td className="px-3 py-2 whitespace-nowrap text-zinc-600 dark:text-zinc-300">
                    {client.activities[0]
                      ? new Date(client.activities[0].createdAt).toLocaleDateString("pt-BR")
                      : "—"}
                  </td>
                  <td className="px-3 py-2">
                    <span className="inline-flex rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700">
                      {client.stage.replaceAll("_", " ")}
                    </span>
                  </td>
                  <td className="px-3 py-2">
                    {client.stage === "IMPORTADO" ? (
                      <button
                        onClick={() => sendToKanban(client.id)}
                        disabled={sendingId === client.id}
                        className="whitespace-nowrap rounded bg-zinc-900 px-3 py-1.5 text-xs font-medium text-white hover:bg-zinc-700 disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900"
                      >
                        {sendingId === client.id ? "Enviando..." : "Enviar para o Kanban"}
                      </button>
                    ) : (
                      <span className="text-xs text-zinc-400">No funil</span>
                    )}
                  </td>
                </tr>
              ))}
              {clients.length === 0 && (
                <tr>
                  <td colSpan={12} className="px-3 py-6 text-center text-zinc-500">
                    Nenhum cliente importado ainda.
                  </td>
                </tr>
              )}
            </tbody>
